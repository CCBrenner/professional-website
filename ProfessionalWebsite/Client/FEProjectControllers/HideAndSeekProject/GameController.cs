...
public class GameController
{
    ...
    public string Status => !GameIsOver ? $"You are {HandleLandingGrammar(CurrentLocation.Name)} the {CurrentLocation.Name}. " +
    ...
    private string Load(string nameOfFileToLoad)
    {
        ...
        foreach (KeyValuePair<string, string> pair in loadedGame.OpponentsInHidingLocations)
            (House.GetLocationByName(pair.Value) as LocationWithHidingPlace).HidingPlace.Clear();
        foreach (KeyValuePair<string, string> pair in loadedGame.OpponentsInHidingLocations)
            (House.GetLocationByName(pair.Value) as LocationWithHidingPlace).HidingPlace.Add(new Opponent(pair.Key));
    ...
    public string ParseInput(string input)
    {
        ...
        {
            if ((CurrentLocation is LocationWithHidingPlace location) && location.HidingPlaceName != "")
            {
                IEnumerable<Opponent> foundOpponents = location.GetHiddenOpponents();
                if (foundOpponents.Count() > 0)
                {
                    FoundOpponents.AddRange(foundOpponents);
                    location.HidingPlace.Clear();
                    return $"You found {foundOpponents.Count()} opponent{House.S(foundOpponents.Count())} hiding {location.HidingPlaceName}.<br>";
                }
                else
                    return $"Nobody was hiding {location.HidingPlaceName}<br>";
            }
            else
    ...
    {
        if (location is LocationWithHidingPlace locationWithHidingPlace && locationWithHidingPlace.HidingPlaceName != "")
            return $"<br>Someone could hide {locationWithHidingPlace.HidingPlaceName}<br>";
        else
            return "";
    }
}
